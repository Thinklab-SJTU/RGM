...
from utils.bi_stochastic import BiStochastic as Sinkhorn
...
class RRWM(nn.Module):
    ...
    def forward(self, M, num_src, mask, ns_src, ns_tgt, v0=None):
        d = M.sum(dim=2, keepdim=True)
        dmax = d.max(dim=1, keepdim=True).values
        assert torch.sum(dmax == 0) == 0
        M = M / dmax
        ...
        for i in range(self.max_iter):
            ...
            v = self.alpha * self.sk(s, mask, ns_src, ns_tgt).transpose(1, 2).reshape(batch_num, mn, 1) \
                + (1 - self.alpha) * v
            n = torch.norm(v, p=1, dim=1, keepdim=True)
