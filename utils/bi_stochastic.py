...
class BiStochastic(nn.Module):
    ...
    def forward_ori(self, s, mask, nrows=None, ncols=None, dummy_row=False, dtype=torch.float32):
        ...
        row_norm_ones = torch.ones(batch_size, s.shape[1], s.shape[1], device=s.device, dtype=s.dtype)
        col_norm_ones = torch.ones(batch_size, s.shape[2], s.shape[2], device=s.device, dtype=s.dtype)

        # row_norm_ones.masked_fill(mask.mask_ii, 0)
        # col_norm_ones.masked_fill(mask.mask_jj, 0)
        ...
        for i in range(self.max_iter):
            ...
            tmp = 1 / sum
            # tmp = tmp.masked_fill(mask.mask_ij, 0)
            s = s * tmp
